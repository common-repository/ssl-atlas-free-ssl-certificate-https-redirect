...
(function ($) {
  $(document).ready(function () {
    ...
    $body.on('click', 'a.primary.next', function (e) {
      if ($(this).hasClass('review-button')) {
          return;
      }
      ...
      var $form = $('.ssl-atlas-container form');
      if ($form.valid()) {
        showLoading($body);
        $form.submit();
      }
        ...
        rules: {
          ssl_atlas_cpanel_username: {
            ...
            remote: {
              ...
              dataType: "json",
              data: {
                action: 'ssl_atlas_cpanel_check_credentials_ajax',
                username: function () {
                  return jQuery("#ssl_atlas_cpanel_username").val();
                },
                password: function () {
                  return jQuery("#ssl_atlas_cpanel_password").val();
                },
              },
              dataFilter: function (response) {
                if (jQuery("#ssl_atlas_cpanel_password").val() != '') {
                  jQuery("#ssl_atlas_cpanel_password").valid();
        ...
        if ($this.prop("checked")) {
          if (baseDomainName !== '') {
            ...
            $.ajax({
              ...
              method: "POST",
              data: {
                'action': 'ssl_atlas_check_for_dns_records',
                'domain': baseDomainName,
                'nonce': $('input[name=ssl_atlas_generate_certificate_nonce]').val()
              },
    ...
    if ($formStep2.length) {
      ...
      $body.on('click', '#frmstep2 .ssl-atlas-domain-verification-variant-container', function (e) {
        e.preventDefault();
        // Set proper selected class to the container
        $('#frmstep2 .ssl-atlas-domain-verification-variant-container').removeClass('selected');
        $(this).addClass('selected');
        // Set hidden input value
        if ($(this).hasClass('http')) {
          // Yes then set HTTP(1)
          $('#ssl_atlas_domain_verification').val('http');
        } else {
          // Else set DNS(2)
          $('#ssl_atlas_domain_verification').val('dns');
        }
      });

      // Prepare click events for variant tabs
      $body.on('click', '#frmstep2 .ssl-atlas-domain-verification-variant-tabs li', function (e) {
        e.preventDefault();
        // Set proper active class to the tab
        $('#frmstep2 .ssl-atlas-domain-verification-variant-tabs li').removeClass('active');
        $(this).addClass('active');

        // Show tab container
        $('.ssl-atlas-domain-verification-variant-tab-container').removeClass('d-none');
        if ($(this).hasClass('http')) {
          // Yes then hide DNS container
          $('.ssl-atlas-domain-verification-variant-tab-container.dns').addClass('d-none');
          $('#frmstep2 .ssl-atlas-steps-container').removeClass('p-0');
        } else {
          // Yes then hide HTTP container
          $('.ssl-atlas-domain-verification-variant-tab-container.http').addClass('d-none');
          $('#frmstep2 .ssl-atlas-steps-container').addClass('p-0');
        }
      });
      ...
      $body.on('click', '#frmstep2 .scan-dns, #frmstep2 .scan-http', function (e) {
        ...
        var $this = $(this),
                ...
                variantContainerEl = $('.ssl-atlas-domain-verification-variant-tab-container.' + variant);
        ...
        $.ajax({
          url: ajaxurl,
          data: {
            'action': 'ssl_atlas_domain_verification',
            'variant': variant,
            'nonce': $('input[name=ssl_atlas_verify_nonce]').val()
          },
          success: function (dataJson) {
            hideLoading($body);
            var data = JSON.parse(dataJson),
            ...
            if (typeof data['status'] !== 'undefined') {
              if (data['status']) {
                ...
                $('.ssl-atlas-domain-verification-variant-tabs li.' + variant).removeClass('error');
                // Disable the current button
                $this.addClass('disabled');
              } else {
                // Show error message and mark variant as error
                $('.ssl-atlas-domain-verification-variant-tabs li.' + variant).addClass('error');
                message = '<div class="message warning">' + data['message'] + '</div>';
    ...
    if ($formStep3.length) {
      ...
      $body.on('click', '#frmstep3 i.copy', function (e) {
        ...
        $.ajax({
          ...
          method: 'GET',
          data: {
            'action': 'ssl_atlas_cert_files',
            'file_name': fileName,
            'nonce': $('input[name=ssl_atlas_install_certificate_nonce]').val()
          },
          success: function (dataJson) {
            var data = JSON.parse(dataJson);
            var message;
            ...
            if (typeof data['status'] !== 'undefined') {
              if (data['status'] && data['file']) {
                // Further actions
                $body.prepend($('.ssl-atlas-copy-certs-wrapper').clone());
                var certWrapper = $('body > .ssl-atlas-copy-certs-wrapper');
      ...
      // Copy certs
      $body.on('click', '.ssl-atlas-copy-certs-wrapper span.copy', function (e) {
        e.preventDefault();
        if (copy('body > .ssl-atlas-copy-certs-wrapper div.body > textarea')) {
          $('body > .ssl-atlas-copy-certs-wrapper div.head > .message.success').removeClass('d-none');
        } else {
          $('body > .ssl-atlas-copy-certs-wrapper div.head > .message.error').removeClass('d-none');
        }
      });

      // Close certs copy container
      $body.on('click', '.ssl-atlas-copy-certs-wrapper span.close', function (e) {
        e.preventDefault();
    ...
    if ($formStep4.length) {
      // Check renew confirm
      $body.on('change', '#ssl_atlas_renew_confirm', function (e) {
        // Enable next button
        if ($(this).is(':checked')) {
    ...
    if ($formSettings.length) {
      ...
      $('#enable_debug').change(function () {
        ...
        $.ajax({
          ...
          method: 'GET',
          data: {
            'action': 'ssl_atlas_settings_debug',
            'enable_debug': $this.prop("checked") ? 1 : 0,
            'nonce': $('input[name=ssl_atlas_settings_nonce]').val()
          },
      ...
      // Settings tab change
      $body.on('click', '#frmSettings .ssl-atlas-settings-tab-container > li', function (e) {
        var $this = $(this);
        var tab = $this.data('tab');

        // Activate tab and container
        $('#frmSettings .ssl-atlas-settings-tab-container > li').removeClass('active');
        $this.addClass('active');
        $('#frmSettings div.ssl-atlas-settings-container').addClass('d-none');
        $('div.ssl-atlas-settings-container.' + tab + '-container').removeClass('d-none');
      });
      ...
      $body.on('click', '#frmSettings a.renew', function (e) {
        ...
        if ($(this).hasClass('disabled')) {
          ...
          showLoading($body);
          $formSettings.prepend('<input type="hidden" name="ssl_atlas_renew_certificate" value="1">');
          $formSettings.submit();
        }
      });
      ...
      $body.on('click', '#frmSettings a.deactivate', function (e) {
        ...
        if (confirm('Are you sure you want to deactivate the plugin')) {
          showLoading($body);
          $formSettings.prepend('<input type="hidden" name="ssl_atlas_deactivate_plugin" value="1">');
          $formSettings.submit();
        }
      });

      // generic form button linked to a hidden field
      $body.on('click', '#frmSettings a.sslatlas-form-button', function (e) {
        e.preventDefault();
        showLoading($body);
  ...
  function initGenericAutoFields() {
    ...
    $('.sslatlas-compare-value-enable').on('keyup', function (e) {
